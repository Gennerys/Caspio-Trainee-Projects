...
GO
:setvar DatabaseName "TTS_1"
:setvar DefaultFilePrefix "TTS_1"
:setvar DefaultDataPath "C:\Users\oleksandr.papirnyk\AppData\Local\Microsoft\VisualStudio\SSDT\TTS"
:setvar DefaultLogPath "C:\Users\oleksandr.papirnyk\AppData\Local\Microsoft\VisualStudio\SSDT\TTS"
...
GO
PRINT N'Rename refactoring operation with key 2dee50de-6c60-4e9f-98cb-17b8ef74ff7d is skipped, element [dbo].[WorkItem].[DateOfCreation] (SqlSimpleColumn) will not be renamed to Timestamp';


GO
PRINT N'Rename refactoring operation with key d0e71f0c-b501-4461-bb50-4c0dd716b596 is skipped, element [dbo].[WorkItem].[UserId] (SqlSimpleColumn) will not be renamed to [AssignedUserId]';
...
GO
ALTER TABLE [dbo].[SubscribedToWorkItem] WITH NOCHECK
...
GO
ALTER TABLE [dbo].[WorkItem] WITH NOCHECK
...
AS
BEGIN
--CAST(FLOOR(CAST([dbo].[WorkItemCurrentState].[Timestamp] as float)) as datetime)
	SET NOCOUNT ON;
	SELECT *
	FROM [dbo].[WorkItemCurrentState]
	WHERE CAST(FLOOR(CAST([dbo].[WorkItemCurrentState].[Timestamp] as float)) as datetime) = CAST(FLOOR(CAST(@SpecifiedDate as float)) as datetime)
...
--SET IDENTITY_INSERT [dbo].[SubscribedToWorkItem] ON
...
--SET IDENTITY_INSERT [dbo].[SubscribedToWorkItem] OFF
GO
...
GO
ALTER TABLE [dbo].[Comments] WITH CHECK CHECK CONSTRAINT [FK_UserCommented_Id];

ALTER TABLE [dbo].[SubscribedToWorkItem] WITH CHECK CHECK CONSTRAINT [FK_SubscribedUser_Id];

ALTER TABLE [dbo].[WorkItem] WITH CHECK CHECK CONSTRAINT [FK_State_Id];

ALTER TABLE [dbo].[WorkItem] WITH CHECK CHECK CONSTRAINT [FK_Area_Id];

ALTER TABLE [dbo].[WorkItem] WITH CHECK CHECK CONSTRAINT [FK_UserAssigned_Id];

ALTER TABLE [dbo].[WorkItem] WITH CHECK CHECK CONSTRAINT [FK_WorkItemType_Id];

ALTER TABLE [dbo].[WorkItem] WITH CHECK CHECK CONSTRAINT [CHECKPriority];

ALTER TABLE [dbo].[WorkItem] WITH CHECK CHECK CONSTRAINT [CHECKEstimate];
