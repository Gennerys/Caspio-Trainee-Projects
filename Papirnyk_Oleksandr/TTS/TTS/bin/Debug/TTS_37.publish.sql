...
BEGIN
	SET NOCOUNT ON;
	INSERT INTO [TTS].[dbo].[WorkItem] ([WorkItemId]
      ,[WorkItemTypeId]
      ,[Title]
      ,[StateId]
      ,[AreaId])
	SELECT TOP (1) 
      ...
      ,[StateId]
      ,[AreaId] 	
  FROM [TTS].[dbo].[WorkItem]
  WHERE  [TTS].[dbo].[WorkItem].[WorkItemId] = @WorkItemId 
  ORDER BY [TTS].[dbo].[WorkItem].[RevisionId] desc

UPDATE [TTS].[dbo].[WorkItem]
SET [TTS].[dbo].[WorkItem].[DateOfClose] = ISNULL([DateOfClose],@DateOfClose), [TTS].[dbo].[WorkItem].[Priority] = ISNULL([Priority],@Priority),
[TTS].[dbo].[WorkItem].[Description] = ISNULL([Description],@Description), [TTS].[dbo].[WorkItem].[Estimate] =  ISNULL([Estimate],@Estimate),
[TTS].[dbo].[WorkItem].[AssignedUserId] = ISNULL([UserId],@UserId), [TTS].[dbo].[WorkItem].[ParentWorkItem] = ISNULL([ParentWorkItem],@ParentWorkItem),
[TTS].[dbo].[WorkItem].[IsDeleted] = ISNULL([IsDeleted],@IsDeleted)
WHERE [TTS].[dbo].[WorkItem].[WorkItemId] = @WorkItemId
AND [TTS].[dbo].[WorkItem].[RevisionId]
in 
